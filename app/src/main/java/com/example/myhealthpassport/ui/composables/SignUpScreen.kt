...
fun SignUpScreen(navController: NavController, auth: FirebaseAuth) {
    ...
    Column(modifier = Modifier.fillMaxSize()
        ...
        ExtendedFloatingActionButton(onClick = {
            ...
            when {
                cleanEmail.isBlank() || cleanPassword.isBlank() -> errorMessage = "Email and password cannot be empty"
                cleanPassword.length < 6 -> errorMessage = "Password must be at least 6 characters"
                cleanPassword != cleanConfirm -> errorMessage = "Passwords do not match"
                else -> {
